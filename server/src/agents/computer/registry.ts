...
/** Runtimes a paired (non-cloud) computer is allowed to advertise. */
const PAIRABLE_ENGINES: ReadonlySet<string> = new Set(['claude', 'codex', 'pi'])
 ...
 *  overrides so the daemon can pass them to the runtime.
 *
 *  When a row has no explicit model, fall back to the deploy-level runtime
 *  default so every BYOA daemon gets a consistent pin — independent of whatever
 *  model the local CLI happens to default to today. */
